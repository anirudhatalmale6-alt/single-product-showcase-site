...
/**
 * A short, readable order reference. Derived from the Stripe session id so
 * the webhook and the success page always produce the SAME ref - that is
 * what makes the confirmation email idempotent.
 */
function refFromSessionId(id, content) {
  const prefix = (content?.brand?.shortName || 'ORD')
    .replace(/[^A-Za-z]/g, '')
    .slice(0, 3)
    .toUpperCase() || 'ORD';
    ...
    line_items: [
      {
        ...
        price_data: {
          ...
          product_data: {
            ...
            description: `${p.spec}. ${p.shortDescription}`.slice(0, 500),
            metadata: { sku: p.sku }
          }
        }
      }
...
function shippingCountries() {
  const raw = (process.env.SHIPPING_COUNTRIES || 'AE,SA,QA,KW,BH,OM,IN,GB,US')
    .split(',')
...
async function orderFromSession(sessionId, content) {
  ...
  return {
    ref: refFromSessionId(session.id, content),
    mode: mode(),
